...
string shape2str(const xt::svector<unsigned int, 4u, std::allocator<unsigned int>, true>& vec)
{
    xt::svector<unsigned long> converted_vec(vec.begin(), vec.end());

    return shape2str(converted_vec);
}

// string shape2str(const xt::svector<size_t>& vec) 
// {
//     xt::svector<unsigned long> converted_vec(vec.begin(), vec.end());
